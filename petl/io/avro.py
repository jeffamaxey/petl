...
class AvroView(Table):
    ...
    def _open_reader(self, source_file):
        ...
        import fastavro
        return fastavro.reader(source_file, **self.avro_args)
    ...
    def _decode_schema(self, avro_reader):
        ...
        return tuple(col['name'] for col in schema_fields)
    ...
    def _read_rows_from(self, avro_reader, header):
        ...
        for i, record in enumerate(avro_reader):
            ...
            if count >= maximum:
                break
            count += 1
            yield self._map_row_from(header, record)
    ...
    def _map_row_from(self, header, record):
        ...
        return (
            ...
            else tuple(record.get(col) for col in header)
        )
...
def _update_field_defs_from(props, row, fields, previous, fill_missing):
    for prop, val in izip_longest(props, row):
        if prop is None:
            break
        dprev = previous.get(f'{prop}_prec')
        fprev = previous.get(f'{prop}_prop')
        fcurr = None
        ...
        if dcurr is not None:
            previous[f'{prop}_prec'] = dcurr
        if fcurr is not None:
            previous[f'{prop}_prop'] = fcurr
...
def _get_definition_from_record(prop, val, fprev, dprev, fill_missing):
    ...
    schema_fields = list(fprev.values())
    tdef = {
        'type': 'record',
        'name': f'{prop}_record',
        'namespace': 'avro',
...
def _fix_missing_headers(table, schema):
    ...
    return table2 if len(cols) >= len(headers) else setheader(table2, headers)
...
def _get_error_details(target, num, err, record, schema):
    ...
    dest = f" output: {target}" if isinstance(target, string_types) else ''
    printed = "failed writing on row #%d: %s\n%s\n schema: %s\n%s"
    return printed % (num, err, dest, schema, example)


def _get_schema_header_names(schema):
    fields = schema.get('fields')
    return [] if fields is None else [field.get('name') for field in fields]
